...
export interface EnhancedFilterValues {
  ano: string;
  tema: string;
  tipo_estudo: string;
}
...
interface EnhancedFilterPanelProps {
  filters: EnhancedFilterValues;
  onChange: (filters: EnhancedFilterValues) => void;
  onClear: () => void;
...
export function EnhancedFilterPanel({ 
  ...
  const hasActiveFilters = filters.ano || filters.tema || filters.tipo_estudo;
  ...
  const handleStudyTypeChange = (type: string) => {
    onChange({ ...filters, tipo_estudo: filters.tipo_estudo === type ? "" : type });
  };
  ...
  return (
    <Card className="p-6">
      ...
      <div className="space-y-6">
        ...
        <div>
          <Label className="text-sm font-medium mb-3 block">Tipo de Estudo</Label>
          <div className="flex flex-wrap gap-2">
            {availableStudyTypes.map((type) => (
              <Badge
                key={type}
                variant={filters.tipo_estudo === type ? "default" : "outline"}
                className="cursor-pointer hover-elevate active-elevate-2"
                onClick={() => handleStudyTypeChange(type)}
                data-testid={`filter-study-type-${type}`}
              >
                {type}
              </Badge>
            ))}
      ...
      {hasActiveFilters && (
        <div className="mt-6 pt-6 border-t">
          ...
          <div className="flex flex-wrap gap-2">
            ...
            {filters.tipo_estudo && (
              <Badge variant="secondary" className="gap-1" data-testid="active-filter-study-type">
                Tipo: {filters.tipo_estudo}
                <button
                  onClick={() => onChange({ ...filters, tipo_estudo: "" })}
                  className="ml-1 hover:text-destructive"
                  aria-label="Remove study type filter"
                >
                  <X className="h-3 w-3" />
                </button>
              </Badge>
